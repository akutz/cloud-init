...
def temporary_hostname(temp_hostname, cfg, hostname_command="hostname"):
    ...
    if (
        not util.is_true(cfg.get("set_hostname"))
...
class DataSourceAzure(sources.DataSource):
    ...
    def bounce_network_with_azure_hostname(self):
        ...
        with temporary_hostname(
            azure_hostname, self.ds_cfg, hostname_command=hostname_command
        ) as previous_hn:
            if previous_hn is not None and util.is_true(
                self.ds_cfg.get("set_hostname")
            ):
                cfg = self.ds_cfg["hostname_bounce"]

                # "Bouncing" the network
                try:
                    return perform_hostname_bounce(
    ...
    def crawl_metadata(self):
        ...
        perform_reprovision_after_nic_attach = (
            ...
            or self._should_reprovision_after_nic_attach(ret)
        )
    ...
    def wait_for_link_up(self, ifname):
        ...
        while True:
            ...
            sleep_duration = 1
            msg = (
                "Link is not up after %d attempts with %d seconds sleep "
            ...
            if self.distro.networking.is_up(ifname):
                msg = (
                    "Link is up after checking after sleeping for %d secs"
                    " after %d attempts" % (sleep_duration, attempts)
                )
                report_diagnostic_event(msg, logger_func=LOG.info)
                return
        ...
        try:
            ...
            while True:
                ...
                with events.ReportEventStack(
                    ...
                    description=(
                        "wait for nic attach after %d nics have "
    ...
    def _should_reprovision_after_nic_attach(self, candidate_metadata) -> bool:
        ...
        ) == "Savable" or os.path.isfile(path):
    ...
    def _negotiate(self):
        ...
        LOG.debug(
            "negotiating with fabric via agent command %s",
